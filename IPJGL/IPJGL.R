    ...
    for (j in 1:rho.steps) {
        for (step in 1:step.max) {
            ...
            for (i in 1:T) {  # 对每一组数据进行更新
                if (i == 1){
                    solve.theta((n * sigma_arr[[i]] + F_arr[[i]] + Q_arr[[i]]) - rho * (theta_arr[[i+1]] + (V_arr[[i]] + W_arr[[i]]) + Z_arr[[i]]),
                            V_arr[[i]],
                            rho,
                            n,
                            lambda2,
                    ...
                    solve.theta((n * sigma_arr[[i]] - F_arr[[i - 1]] + Q_arr[[i - 1]]) - rho * (theta_arr[[i - 1]] - (V_arr[[i - 1]] + W_arr[[i - 1]]) + Z_arr[[i]]),
                        V_arr[[i - 1]],
                        rho,
                        n,
                        lambda2,
                    ...
                    solve.theta((n * sigma_arr[[i]] + F_arr[[i]] - F_arr[[i - 1]] + Q_arr[[i]]) - rho * (theta_arr[[i - 1]] + theta_arr[[i + 1]] + (V_arr[[i]] + W_arr[[i]]) - (V_arr[[i - 1]] + W_arr[[i - 1]]) + Z_arr[[i]]),
                        V_arr[[i]] + V_arr[[i - 1]],
                        rho,
                        n,
                        lambda2,
